...
class SystemPromptBuilder:
    ...
    def add_code_execution_guide(self) -> "SystemPromptBuilder":
        ...
        section = [
            ...
            "- Intermediate script files (.py, .sh, etc.): **must set notify_frontend=false** (these are code to be executed, not final output)",
            "- **Non-script / document files (.md, .pdf, .docx, .pptx, .xlsx, .csv, .txt, .json, .html, .xml, .yaml, .png, .jpg, etc.): MUST set notify_frontend=true**",
            "  - These are final deliverables the user wants to see — they must be displayed in the frontend!",
            "  - Example: `write_file('report.md', content, notify_frontend=true)`",
            "  - Example: `write_file('analysis.json', data, notify_frontend=true)`",
            "- Files generated by `exec` (e.g. python script.py produces output.pdf): automatically notify the frontend, no setup needed",
            "",
            "**shell_exec vs exec:**",
            ...
            "- `write_file` with `notify_frontend=true`: triggers frontend file display (use for non-script files like .md, .pdf, .docx, .txt, .csv, .json, etc.)",
            "- `write_file` with `notify_frontend=false`: hidden from frontend (use ONLY for intermediate .py/.sh scripts)",
            "- `exec` generated files: auto-detected and notifies frontend automatically",
