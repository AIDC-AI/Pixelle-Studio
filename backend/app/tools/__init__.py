...
1. write_file("script.py", "code content...", notify_frontend=false)  # Intermediate script
2. exec("python script.py")  # Generate final files (PDF, Excel, etc.)

notify_frontend parameter — CRITICAL:
- **false**: ONLY for intermediate script/code files (.py, .sh) that will be executed, NOT shown to user
- **true**: For ALL non-script / document / data files that the user should see:
  .md, .txt, .json, .csv, .html, .xml, .yaml, .docx, .pptx, .xlsx, .pdf, .png, .jpg, etc.
  Example: write_file("report.md", content, notify_frontend=true)
  Example: write_file("data.json", json_str, notify_frontend=true)
- Files generated by exec automatically notify the frontend, no setup needed

Rule of thumb: if the file is NOT a script to be executed, set notify_frontend=true!
...
""",
            "parameters": {
                ...
                "properties": {
                    ...
                    "notify_frontend": {
                        "type": "boolean",
                        "description": "Whether to notify frontend to display this file. Set false ONLY for intermediate scripts (.py, .sh). Set true for ALL non-script files (.md, .pdf, .docx, .txt, .csv, .json, .html, etc.) — these are user-facing deliverables!",
                        "default": False
                    }
                },
