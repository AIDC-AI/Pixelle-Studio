...
![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/e721bc2c-e22a-4904-a676-749412da46a2.png)
...
我的项目源码在：[https://code.alibaba-inc.com/qingli.hql/mcp-workflow](https://code.alibaba-inc.com/qingli.hql/mcp-workflow)，欢迎大家钉钉交流。
...
![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/d626307e-952d-4034-b84e-c0d299831c2f.png)

![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/315fd8db-69c4-457d-aeb7-d386b3a2a2e0.png)

![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/3b06310b-ce27-4380-a647-c08e94446ceb.png)

![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/0ee9934f-ee11-415b-9e32-51b1301eecae.png)
...
![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/e8709fc8-64df-45b8-886c-29b92c216a82.png)

![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/d85fe6e7-91b9-4402-94a1-6be98ad85b2c.png)
...
![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/d6f809ba-7c51-47e8-b359-f229fc40310d.png)
...
Loader --> Repo : Read Files
Agent --> Runner : Execute Python Code
Runner --> WorkDir : R/W Files
Runner --> Repo : Access Helper Scripts
Runner --> Agent : Stdout/Stderr/Result
...
![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/5dbd9d0b-cdc7-4676-b050-02911b639ab6.png)
...
在开发过程中，我的架构经历了一次重大的自我否定与迭代，最终形成了一套独特的**Hybrid Tool Calling**机制。
...
### 5.4 终极方案：Hybrid 模式与 Context 中转
...
这是最关键的技术细节。在后端 `backend/app/agent.py` 处理流式响应时，我增加了一个**实时提取器**。
当系统检测到 `<execute>` 标签闭合时，会立即将里面的代码提取出来，并**存入到当前会话的 Agent Context 队列中**。
...
#### 第三步：Trigger（无参触发）
Agent 写完代码后，只需要调用一个**不带任何参数**的 `execute_code()` 工具。
这就像是一个"确认按钮"。Agent 只是在告诉系统："我代码写好了（在正文里），请执行它。"
...
1.  **忘记按按钮**：模型写完代码后，有时会"意满离"，觉得自己已经回答完了，忘记调用 `execute_code`。
...
![image.jpeg](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/5VLqXLby4aQoVqX1/img/e1ab6846-f63d-4f23-9093-38273843cfb7.jpeg?x-oss-process=image/crop,x_0,y_182,w_1024,h_842/ignore-error,1)
...
### 6.2 Agent Loop 的进化：拥抱 OpenAI Agents SDK

在早期的版本中，我不得不手写一个复杂的 `while` 循环来管理 Agent 的状态机（调用 LLM -> 解析 -> 执行 -> 构造历史 -> 再次调用）。这不仅代码冗余，而且很难处理流式输出（Streaming）。

在最新版中，我引入了 **OpenAI Agents SDK**，将核心循环简化为了一行代码：

```python
# 截取自 backend/app/agent.py
...
1.  **自动化的 Context 维护**：SDK 会自动处理 Tool Call 和 Tool Result 的历史记录，我不再需要手动拼接 `messages` 数组。
2.  **事件驱动（Event-Driven）**：通过 `result.stream_events()`，我可以像监听事件一样处理 Agent 的思考过程。

```python
async for event in result.stream_events():
    if isinstance(event, RawResponsesStreamEvent):
        # 实时捕获 LLM 的打字机效果，并提取 <execute> 代码块
        self._extract_execute_blocks(event.content)
...
### 6.3 关键机制：从"伪造消息"到"标准回环"

在旧架构中，为了让 LLM 看到代码执行结果，我不得不把 stdout 伪装成一条 `User Message` 塞回历史记录。这种 Hack 做法虽然有效，但容易干扰 LLM 的角色认知。

拥抱 Agents SDK 后，这一切变得标准化了：

1.  **Tool Role**：执行结果现在以标准的 `role: "tool"` 消息存在于上下文中。
2.  **透明的中间态**：虽然 SDK 自动接管了闭环，但我依然可以通过 `stream_events` 钩子，把 `execution_result`、`skill_loaded` 等中间状态实时推送到前端 WebSocket。
...
如果直接把这些工具丢给 Agent，让它"做一个短视频"，它可能会不知所措：先做图还是先做音频？音频和图片怎么对应？
...
2.  **入参和出参都要短**：
    *   **入参**：能让模型自己生成的，就不要传参。比如我的 `execute_code` 改成了无参调用（代码在正文），彻底解决了长代码传参的噩梦。
