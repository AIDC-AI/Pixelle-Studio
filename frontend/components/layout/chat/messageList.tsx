...
// 系统操作类型的消息
const SYSTEM_OPERATION_TYPES = ['code', 'execution_result', 'skill_loaded', 'iteration', 'log', 'evaluation', 'advice', 'system', 'tool_call', 'tool_result', 'thinking'];
...
const MessageItem = memo(({ msg, isLast, onFilePreview }: { 
    msg: Message; 
    isLast: boolean;
    onFilePreview?: (file: OutputFile) => void
}) => {
...
// 虚拟化的消息组渲染器
const VirtualMessageGroup = memo(({ group, isLastGroup, onFilePreview }: { 
    group: MessageGroup
    isLastGroup: boolean 
    onFilePreview?: (file: OutputFile) => void
}) => {
...
const MessageList: React.FC<IProps> = (props) => {
    const { messages, currentScript, isProcessing, onFilePreview } = props;  
